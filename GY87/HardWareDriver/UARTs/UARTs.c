...
volatile unsigned char u1_rx_buf[200];
volatile unsigned char u1_rx_length;
volatile u8 u1_rx_over;
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
	{
		...
		u1_rx_buf[u1_rx_length]=data;
		u1_rx_length++;

		if(u1_rx_length>1)
		{
			if(u1_rx_buf[u1_rx_length-2]==0x0D && u1_rx_buf[u1_rx_length-1]==0x0A)
			{
				u1_rx_over = 1;
			}
		}
